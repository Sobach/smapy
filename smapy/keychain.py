...
from smapy.http_utilities import vk_auth, fb_auth, ok_auth
import pprint
import datetime
import pickle
import hashlib
...
class KeyChain(object):
    ...
    def assign(self, net, token, validate = True, check = False):
        self._networks = CONNECTORS.keys()
        if validate and not self._token_format_validator(net, token):
            logging.error(u'KEYCHAIN: Token wasn\'t validated by service.')
            return
        if net in self._networks:
            if check and not self._checker(net, token):
                logging.error(u'KEYCHAIN: Token wasn\'t validated by service.')
                return
            ...
            if net == 'yt':
                ...
                self._keys['yt'] = token
            self.autocomplete()
        elif net in ('raw_vk', 'raw_fb', 'raw_ok'):
            if check:
                if net == 'raw_vk' and not self._checker('vk', vk_auth(token)):
                    logging.error(u'KEYCHAIN: Token wasn\'t validated by service.')
                    return
                if net == 'raw_fb' and not self._checker('fb', fb_auth(token)):
                    logging.error(u'KEYCHAIN: Token wasn\'t validated by service.')
                    return
                if net == 'raw_ok' and not self._checker('ok', ok_auth(token)[0]):
                    logging.error(u'KEYCHAIN: Token wasn\'t validated by service.')
                    return
            self._keys[net] = token
            self.autocomplete()
        else:
            logging.error(u'KEYCHAIN: Token wasn\'t assigned: unknown network identifier.')
            return
    ...
    def check(self, net = None, token = None):
        ...
        if not net and token:
            ...
            elif net == 'ok' and 'raw_ok' in self._keys:
    ...
    def _checker(self, network, key):
        ...
        if not self._token_format_validator(network, key):
    ...
    def _token_format_validator(self, net, token):
        ...
        if net not in token_formats:
            if isinstance(token, (str, unicode)):
                ...
                return False
        if isinstance(token, dict) and not token_formats[net].difference(set(token.keys())):
            return True
        else:
